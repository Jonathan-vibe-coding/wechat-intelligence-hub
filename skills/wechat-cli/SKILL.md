---
name: wechat-cli
description: Read-only access to the user's local WeChat data through rion-wechat-cli. Use when the user asks to inspect a recent chat, search messages, resolve a contact, review a conversation timeline, or draft a reply from current context without sending anything.
---
...
1. On first use, run `reader.sh self-test`, then `reader.sh setup --pretty`. After upgrades or failed reads, run `reader.sh --pretty doctor`. Continue with database-backed reads only when `live_database_read_ok=true`; notification-only mode must be labeled `incoming_preview_only`.
